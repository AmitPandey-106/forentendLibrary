...
export default function AdminBorrowRequests() {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        const response = await fetch("http://localhost:8000/borrow-requests/pending");
    ...
    const fetchWaitlist = async () => {
      try {
        const response = await fetch("http://localhost:8000/get-waitlist");
        const data = await response.json();
        setWaitlist(data);
      } catch (error) {
    ...
    const markRequestsViewed = async () => {
      try {
        await fetch("http://localhost:8000/borrow-requests/mark-viewed", { method: "POST" });
  ...
  const handleRequestAction = async (requestId, action) => {
    try {
      const response = await fetch("http://localhost:8000/user-borrow/request", {
        method: "POST",
