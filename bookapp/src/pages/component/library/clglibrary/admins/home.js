...
export async function getServerSideProps() {
  const res = await fetch("http://localhost:8000/all-members");
  const data = await res.json();

  const res2 = await fetch("http://localhost:8000/all-books");
  const data2 = await res2.json();

  const res3 = await fetch('http://localhost:8000/all-borrow-books');
...
export default function HomePage({ memberLength, bookslen, allborrowed }) {
  ...
  useEffect(() => {
    const fetchMostBorrowedBooks = async () => {
      try {
        const response = await fetch("http://localhost:8000/most-borrowed"); // Replace with your API endpoint
  ...
  const fetchTommorow = async () => {
    try {
      setLoading(true);
      const response = await fetch("http://localhost:8000/tmr-due-books");
  ...
  const fetchDueBooks = async () => {
    try {
      setLoading(true);
      const response = await fetch("http://localhost:8000/today-due-books");
      const data = await response.json();
